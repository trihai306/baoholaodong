...
<nav class="flex text-sm" aria-label="Breadcrumb">
    <ol class="inline-flex items-center space-x-2">
        ...
        <li><a href="{{ route('products.index') }}" class="text-dark-500 hover:text-primary-500 transition">Sản phẩm</a></li>
        <li class="text-dark-300">/</li>
        <li class="text-dark-800 font-medium">{{ $category->name }}</li>
    </ol>
</nav>
@endsection
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="lg:grid lg:grid-cols-4 lg:gap-8">
                    ...
                    <div class="p-2">
                        @foreach($categories as $cat)
                            <a href="{{ route('products.category', $cat->slug) }}"
                               ...
                               {{ $cat->id == $category->id || $category->parent_id == $cat->id ? 'bg-primary-50 text-primary-600' : 'text-dark-700 hover:bg-dark-50 hover:text-primary-600' }}">
                                <span>{{ $cat->name }}</span>
                                @if($cat->children->where('is_active', true)->count())
                                    <span class="text-xs text-dark-400">{{ $cat->children->where('is_active', true)->count() }}</span>
                                @endif
                            </a>
                            @if($cat->children->where('is_active', true)->count())
                                <div class="ml-3 border-l-2 border-dark-100 pl-2 mb-1">
                                    @foreach($cat->children->where('is_active', true)->sortBy('sort_order') as $child)
                                        <a href="{{ route('products.category', $child->slug) }}"
                                           ...
                                           {{ $child->id == $category->id ? 'bg-primary-50 text-primary-600' : 'text-dark-500 hover:bg-dark-50 hover:text-primary-600' }}">
            ...
            <div class="lg:col-span-3">
                <div class="mb-8">
                    <h1 class="text-2xl lg:text-3xl font-extrabold text-dark-900 mb-2">{{ $category->name }}</h1>
                    @if($category->description)
                        <p class="text-dark-500">{{ $category->description }}</p>
                    @endif
                </div>
                ...
                <div class="text-center py-20 bg-white rounded-2xl border border-dark-100">
                    ...
                    <p class="text-dark-500 text-lg">Chưa có sản phẩm trong danh mục này.</p>
                </div>
                @endif
            </div>
        </div>
