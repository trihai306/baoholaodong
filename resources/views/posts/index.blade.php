...
<nav class="flex text-sm" aria-label="Breadcrumb">
    <ol class="inline-flex items-center space-x-2">
        ...
        <li class="text-dark-800 font-medium">Tin tức</li>
    </ol>
</nav>
@endsection
...
<section class="py-10 lg:py-14">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-12">
            <div class="inline-flex items-center px-4 py-1.5 bg-blue-50 text-blue-600 text-xs font-semibold rounded-full mb-4 uppercase tracking-wider border border-blue-100"><i class="fas fa-newspaper mr-1.5"></i>Blog</div>
            <h1 class="text-3xl lg:text-4xl font-extrabold text-dark-900 mb-3">Tin tức & Kiến thức</h1>
            <p class="text-dark-500 max-w-2xl mx-auto">Cập nhật thông tin mới nhất về bảo hộ lao động và an toàn lao động</p>
        </div>

        @if($posts->count())
        <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
            @foreach($posts as $post)
            <article class="bg-white rounded-2xl overflow-hidden border border-dark-100 hover:shadow-md transition-all duration-300 hover:-translate-y-1 group flex flex-col">
                <div class="aspect-video overflow-hidden">
                    ...
                    @else
                        <div class="w-full h-full bg-gradient-to-br from-dark-100 to-dark-200 flex items-center justify-center">
                            <i class="fas fa-newspaper text-5xl text-dark-300"></i>
                        </div>
                    @endif
                </div>
                <div class="p-6 flex flex-col flex-1">
                    ...
                    <h2 class="text-lg font-bold text-dark-800 group-hover:text-primary-600 transition line-clamp-2 mb-3">
                        <a href="{{ route('posts.show', $post->slug) }}">{{ $post->title }}</a>
                    </h2>
                    @if($post->excerpt)
                        <p class="text-dark-500 text-sm line-clamp-3 mb-4">{{ $post->excerpt }}</p>
                    @endif
        ...
        <div class="text-center py-20 bg-white rounded-2xl border border-dark-100">
            <i class="fas fa-newspaper text-5xl text-dark-200 mb-4"></i>
            <p class="text-dark-500 text-lg">Chưa có bài viết nào.</p>
        </div>
        @endif
    </div>
</section>
